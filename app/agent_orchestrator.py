...
def _create_prompt_template(enable_10k_rag: bool) -> PromptTemplate:
    """Create the ReAct prompt template with dynamic examples."""
    examples_text = _build_prompt_examples(enable_10k_rag)
...
def create_financial_agent() -> AgentExecutor:
    """
    Creates and configures the financial analyst agent with all tools and settings.
    
    Returns:
        AgentExecutor: Configured agent ready for queries
    ...
    # 3. Create custom ReAct prompt optimized for GPT-4o-mini's formatting
    prompt = _create_prompt_template(enable_10k_rag)
...
def execute_query(agent_executor: AgentExecutor, question: str) -> dict:
    """
    Execute a query using the financial agent.
    ...
    Returns:
        dict: The agent's response containing the output
    """
